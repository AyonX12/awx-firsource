...
mkdir -p "$SEED/network_inventory_mvp_v3/baseline"
printf 'Seat ID (do not edit),Agent Name\n1,old\n' > "$SEED/network_inventory_mvp_v3/baseline/floor_33_import_template.csv"
...
python3 "$ROOT/scripts/publish_csv_to_git.py" \
  --repo-url "$BARE" \
  --branch main \
  --project-subdir network_inventory_mvp_v3 \
  --source-file "$UPDATED" \
...
grep '1,new' "$VERIFY/network_inventory_mvp_v3/baseline/floor_33_import_template.csv" >/dev/null
