...
* Targeted scans only make claims inside target_hosts; therefore a miss is
...
def main() -> int:
    ...
    parser.add_argument("--everest-id", default="")
    parser.add_argument("--target-hosts", default="")
    parser.add_argument("--scan-scope", choices=("targeted", "full"), default="targeted")
    ...
    if args.everest_id.strip():
        ...
        baseline = [
            row
            for row in baseline
            if (row.get("Everest Id") or "").strip().lower() == wanted
        ]
        if not baseline:
    ...
    if args.everest_id.strip() and len(results) == 1:
        summary["target_result"] = results[0]
