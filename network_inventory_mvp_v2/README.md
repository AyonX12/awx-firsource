...
## Excel seating-map update

After MAC comparison, the playbook can update the physical seating table for users whose status is `PORT_MOVED` or `SWITCH_MOVED` (including `+VLAN_CHANGED` variants).

Expected workbook:

```text
baseline/floor_33_import_template.xlsx
```
...
Example AWX variables:

```yaml
target_hosts: "asw11:asw12"
client_name: "Reprise"
...
```yaml
seating_sheet: "Seats"  # omit to auto-detect the sheet by headers
# seating_workbook_path: "/persistent/path/floor_33_import_template.xlsx"
# seating_output_path: "/persistent/path/floor_33_import_template_updated.xlsx"
```

The default input is `baseline/floor_33_import_template.xlsx`. The default output is created inside the AWX job workspace as `floor_33_import_template_updated.xlsx`.

**Important:** AWX execution-node `/tmp` storage is temporary. If the updated workbook must survive the job, set `seating_output_path` to a persistent path mounted into the Execution Environment, or add a later workflow step that uploads the generated workbook to the system where the master seating file is stored.

The Excel update requires `openpyxl>=3.1.0` in the Execution Environment. The playbook performs a preflight check and gives a clear error if it is missing.
